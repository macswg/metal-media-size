...
export function removalHeaderText(chunk: ExportChunk, o: RemovalGuiOptions): string {
  ...
  const scope =
    chunk.basePrefix === ''
      ? [
          '   This is ONE job for the whole run, so the right-hand folder is the archive',
          '   ROOT and the include filter is what narrows it. Every path it names was',
          '   selected; nothing outside the filter is compared, let alone touched. If the',
          '   filter block is ever emptied or edited, this job stops being narrow, so',
          '   check the row count after Compare, every time.',
        ]
      : [];

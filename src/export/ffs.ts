...
export interface RemovalGuiOptions {
  ...
  note?: string | null;
  /**
   * The programmed-media cross-check as it stood when this job was generated,
   * or null when no capture was loaded.
   *
   * NULL IS THE LOUD CASE and the banner says so in words. An export with no
   * cross-check and an export whose cross-check found nothing to protect
   * produce identical file lists; only this line tells them apart, and the
   * operator reading the job is the last person who can.
   */
...
export function removalHeaderText(chunk: ExportChunk, o: RemovalGuiOptions): string {
  ...
  // Sixty-five folder names on one line is not information, it is a wall. Past
  // a handful the count is the useful fact and the companion manifest carries
  // the detail -- it lists every literal path anyway.
  const songs =
    chunk.songFolders.length > 6
      ? `${chunk.songFolders.length} song folders (listed in ${o.manifestFileName})`
      : chunk.songFolders.join(', ');

  // Under the single-job layout the pair is the archive root, so the include
  // filter is the ONLY thing narrowing what this job can touch. That is worth
  // saying out loud to the person about to press Compare rather than leaving
  // them to infer it from a path.
  const scope =
    chunk.basePrefix === ''
      ? [
          '   This is ONE job for the whole run: its folder pair is the delivery folder',
          '   itself rather than a single song, and the include filter is what narrows it.',
          '   Every path it names was selected; nothing outside the filter is compared, let',
          '   alone touched. If the filter block is ever emptied or edited, this job stops',
          '   being narrow, so check the row count after Compare, every time.',
        ]
      : [];

  // With a blank right-hand folder the FIRST thing the operator has to do is
  // supply one, and getting it wrong by a level is the mistake to guard
  // against: the include items are anchored at the root of whatever folder is
  // chosen, so they only match if that folder is the delivery folder itself.
  const blankRight = chunk.pairRightFolder === '';
  const example = chunk.includes[0] ?? '';
  const rightFolderLines = blankRight
    ? [
        '   The right-hand folder is DELIBERATELY EMPTY in this file. Set it in',
        '   FreeFileSync to the delivery folder on the machine you are running this',
        `   on. When scanned, that folder was:  ${chunk.baseFolder}`,
        '',
        '   IT MUST BE THAT FOLDER ITSELF, not its parent and not a subfolder. The',
        '   filter items below are anchored at the root of whatever you choose, e.g.',
        `      ${example}`,
        ...
        '   file names and it would find those instead, which is not. Read the paths',
        '   in the Compare list before you sync, not just the count.',
      ]
    : [`   The right-hand folder is ${chunk.baseFolder}`];

  // The cross-check, stated either way round. See ProgrammedJobSummary.
  const p = o.programmed;
  const programmedLines = p
    ? [
        '   PROGRAMMED-MEDIA CROSS-CHECK: IN FORCE.',
        `   ${p.protectedVersions} version(s) the show is cued to play were held back from this`,
        ...
        `   ${p.matchedNames} of ${p.totalNames} media names matched an archive asset` +
          (p.unmatchedNames > 0
            ? `; ${p.unmatchedNames} did not, and protected nothing. See the export report.`
            : '.'),
        '',
        '   A CAPTURE IS A POINT IN TIME. It records what the show played when it was',
        '   taken, not what it plays now. If the show has been re-programmed since,',
        '   take a fresh capture before running this job.',
      ]
    : [
        '   PROGRAMMED-MEDIA CROSS-CHECK: NOT APPLIED.',
        '   No show-file capture was loaded when this job was generated, so NOTHING',
        '   here has been checked against what the show actually plays. This list is',
        '   what the version-supersession rules alone consider old.',
        '   An empty cross-check is not a clean bill of health. To apply one, put a',
        '   Susan summary export into programmed_media_crosscheck/ and generate again.',
      ];
  ...
  const lines = [
    ...
    ...programmedLines,
    '',
    ...(blankRight ? ['   SET THE RIGHT-HAND FOLDER BEFORE YOU DO ANYTHING ELSE.', ''] : []),
    `   Run:      ${o.runId}   (generated ${o.generatedAt})`,
    o.chunkCount === 1 && chunk.index === 1
      ? '   Job:      the only one in this run'
    ...
    '   BEFORE YOU RUN IT',
    ...(blankRight
      ? ['   0. Set the right-hand folder. It is empty in this file on purpose.']
      : []),
...
export function buildRemovalGui(chunk: ExportChunk, o: RemovalGuiOptions): string {
  ...
  return renderFfsGui({
    ...
    // `pairRightFolder` is '' when the operator is setting the folder in
    // FreeFileSync: the job is being run against a different mount of the same
    // delivery folder, and a path that is right on the scanning machine is a
    // path that has to be noticed and corrected on the other one. The include
    // patterns are anchored and relative, so they bind to whatever is chosen.
    pairs: [{ left: o.emptyLeftFolder, right: chunk.pairRightFolder }],
  });
}
...
export function buildChunkManifest(
  ...
  const lines = [
    ...
    `# Scan root:  ${root}`,
    chunk.pairRightFolder === ''
      ? '# Pair right: (blank in the job -- set it in FreeFileSync before running)'
      : `# Pair right: ${chunk.pairRightFolder}`,
    ...
    ...(chunk.pairRightFolder === ''
      ? [
          '#',
          '# The paths below are written against the SCAN ROOT above, which is where',
          '# these files were seen. The job itself matches on the part after that root,',
          "# so it will act on the same files under whatever folder you point it at --",
          '# provided that folder is the delivery folder itself.',
        ]
      : []),
    '',
  ];
